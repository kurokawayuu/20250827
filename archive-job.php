...
// デバッグ情報をログに出力（本番環境では削除してください）
error_log('Archive Job Debug: paged=' . $paged . ', found_posts=' . $custom_query->found_posts . ', max_num_pages=' . $custom_query->max_num_pages);

// 404エラーを防ぐため、ページが存在しない場合の処理
if ($paged > 1 && $paged > $custom_query->max_num_pages && $custom_query->max_num_pages > 0) {
    // 存在しないページの場合は404にしない（最後のページを表示）
    $args['paged'] = $custom_query->max_num_pages;
    $custom_query = new WP_Query($args);
    $paged = $custom_query->max_num_pages;
}
...
<div class="job-listing-wrapper">
 ...
    <h1 class="page-title">放課後等デイサービスの<?php if (!empty($conditions)): ?>
...
$is_favorite = is_array($favorites) && in_array(get_the_ID(), $favorites);
                ...
                <div class="pagination">
                    ...
                    // カスタムURLパスでページネーションを修正
                    $current_url = $_SERVER['REQUEST_URI'];
                    
                    // ページネーション用のベースURLを構築
                    if (strpos($current_url, '/page/') !== false) {
                        $base_url = preg_replace('/\/page\/[0-9]+\/?/', '', $current_url);
                    } else {
                    ...
                    $pagination_args['base'] = home_url($base_url . '/page/%#%/' . $query_string);
