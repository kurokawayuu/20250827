...
function set_prefecture_seo_data() {
    ...
    if ($prefecture_slug) {
        ...
        if (isset($prefectures[$prefecture_slug])) {
            ...
            // タイトルタグ
            $title = "放課後等デイサービス・児童発達支援 {$pref_name}の教室一覧｜{$site_name}";
            
            // メタディスクリプション
            $description = "{$pref_name}の放課後等デイサービス・児童発達支援教室を一覧でご紹介。こどもプラス加盟教室の所在地・連絡先・求人情報をまとめて確認できます。{$pref_name}で放課後等デイサービスをお探しなら{$site_name}へ。";
            ...
            add_action('wp_head', function() use ($title, $description, $current_url, $pref_name) {
                ...
                $structured_data = array(
                    ...
                    'about' => array(
                        '@type' => 'Service',
                        'name' => '放課後等デイサービス',
                        'description' => $pref_name . 'の放課後等デイサービス教室一覧',
                        'areaServed' => array(
                            '@type' => 'Place',
                            'name' => $pref_name
                        )
...
function enhanced_prefecture_seo() {
    ...
    if ($prefecture_slug) {
        ...
        if (isset($prefectures[$prefecture_slug])) {
            ...
            // タイトルタグの最適化（県名を複数回使用）
            $title = "{$pref_name}の放課後等デイサービス教室一覧【{$classroom_count}校】| {$pref_name}で運動療育なら{$site_name}";
            
            // メタディスクリプションの最適化（県名を自然に3-4回含める）
            $description = "{$pref_name}で放課後等デイサービスをお探しなら運動療育専門の{$site_name}へ。{$pref_name}県内{$classroom_count}校の教室で発達障害・ADHDのお子さまを支援。{$pref_name}各地域の住所・連絡先・求人情報を掲載。児童発達支援も対応。";
            ...
            add_action('wp_head', function() use ($title, $description, $prefecture_slug, $pref_name, $classroom_count, $site_name) {
                ...
                $structured_data = array(
                    ...
                    'about' => array(
                        ...
                        'description' => "{$pref_name}県内の放課後等デイサービス・児童発達支援教室",
                    ...
                    'mainEntity' => array(
                        '@type' => 'ItemList',
                        'name' => "{$pref_name}の放課後等デイサービス教室",
                        'description' => "{$pref_name}県内で運動療育を行う放課後等デイサービス教室の一覧",
                    ...
                    var prefectureDescription = `
                        <div style="background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); 
                            ...
                            <p style="font-size: 14px; 
                                ...
                                ${prefectureName}内${classroomCount}校のこどもプラス加盟教室では、
        ...
        if (isset($prefectures[$prefecture_slug])) {
            ...
            add_action('wp_head', function() use ($pref_name, $classroom_count) {
                ...
                jQuery(document).ready(function($) {
                    ...
                    var $h1 = $('h1').first();
                    if ($h1.length) {
                        $h1.html('<?php echo esc_js($pref_name); ?>の放課後等デイサービス教室一覧');
                    }
                });
                </script>
                <?php
